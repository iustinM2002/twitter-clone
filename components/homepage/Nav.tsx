...
import { faHouseUser,faHashtag, faEnvelope} from '@fortawesome/free-solid-svg-icons';
...
const Nav:NextPage = ():JSX.Element => {
  ...
  return (
    <nav className="min-h-[100vh] flex flex-col w-[20%] items-start  lg:min-h-[10%]  bg-white  semi-tall:hidden lg:w-[5rem] fixed z-10 left-0">
      <ul className="nav-links flex flex-col mx-[2rem] w-full text-left">
         ...
         <li onClick={() => setNavIndex(0)} className={liStyle}><FontAwesomeIcon className={iconStyle} icon={faHouseUser}/><span className={spanClass}>Home</span> </li>
         ...
         <li onClick={() => setNavIndex(6)} className={liStyle}><FontAwesomeIcon className={iconStyle} icon={faHouseUser}/> <span className={spanClass}> Profile</span></li>
     </ul>
 </nav>
  )
}
